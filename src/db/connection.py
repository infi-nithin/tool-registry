...
async def init_engine(config: Optional[DatabaseConfig] = None) -> AsyncEngine:
    ...
    print("Async DB URL:", cfg.async_database_url)
...
async def get_db_session() -> AsyncGenerator[AsyncSession, None]:
    ...
    if async_session_factory is None:
        print("Session factory not initialized. Initializing now...")
    ...
    async with async_session_factory() as session:
            ...
            yield session
        # except Exception:
        #     print("Error committing transaction. Rolling back session.")
