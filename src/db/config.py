...
class DatabaseConfig:
    ...
    port: int = field(default_factory=lambda: int(os.getenv("DB_PORT", "5433")))
    database: str = field(default_factory=lambda: os.getenv("DB_NAME", "mcp_db"))
    ...
    def from_url(cls, url: Optional[str] = None) -> "DatabaseConfig":
        ...
        Example:
            >>> config = DatabaseConfig.from_url("postgresql://user:pass@localhost:5433/dbname")
        """
    ...
    def async_database_url(self) -> str:
        ...
        logger.info(f"Async DB URL: {url}")
        return url
